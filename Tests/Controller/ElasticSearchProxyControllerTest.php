...
class ElasticsearchProxyControllerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function buildController($params = array())
    {
        ...
        $controllerBuilder = $this->getMockBuilder('Xola\ElasticsearchProxyBundle\Controller\ElasticsearchProxyController');
    ...
    public function testAddAuthFilter_singleValidFilter()
    {
        ...
        $this->assertEquals(0, $filterCounter['applied'], 'Applied count should be zero as there were no applicable queries where filter can be applied');
    ...
    public function testAddAuthFilter_multipleValidFilters()
    {
        ...
        $this->assertEquals(2, $filterCounter['applied'], 'Applied count should be zero as there is one applicable query where filter cannot be applied');
        $this->assertEquals(0, $filterCounter['notApplied'], 'notApplied count should be one as there is one applicable query where filter cannot be applied');
        $this->assertEquals('123', $data['facets']['gross']['facet_filter']['query']['filtered']['filter']['bool']['must'][0]['term']['seller.id'], 'Filter should get added correctly within must clause of bool filter');
        $this->assertEquals('123', $data['facets']['fees']['facet_filter']['query']['filtered']['filter']['bool']['must'][0]['term']['seller.id'], 'Filter should get added correctly within must clause of bool filter');
    }
}